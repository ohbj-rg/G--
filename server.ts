...
async function startServer() {
  ...
  // Direct Message & Blind Dating AI smart reply endpoint
  app.post('/api/chat-reply', async (req, res) => {
    try {
      ...
      if (!ai) {
        return res.json({ reply: null, reason: 'NO_API_KEY' });
      }
...
5. 너무 길지 않게 1~3문장 내외의 자연스러운 실시간 모바일 메신저 스타일로 답장하세요.`;
      ...
      if (!replyText) {
        return res.json({ reply: null, reason: 'EMPTY_RESPONSE' });
      }

      return res.json({ reply: replyText });
    } catch (error) {
      console.error('Error in /api/chat-reply:', error);
      return res.json({ reply: null, error: String(error) });
    }
  });
